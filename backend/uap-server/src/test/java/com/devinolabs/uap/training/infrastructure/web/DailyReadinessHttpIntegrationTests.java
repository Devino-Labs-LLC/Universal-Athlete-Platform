...
@Import({ TestcontainersConfiguration.class, DailyReadinessHttpIntegrationTests.MutableClockConfig.class })
class DailyReadinessHttpIntegrationTests {
	...
	@Test
	void readinessEndpointsRequireAuthAndReturnExplainableAssessment() throws Exception {
		AccountId accountId = athlete();

		mockMvc.perform(get("/api/v1/training/readiness/history")
						.param("startDate", "2026-07-01")
						.param("endDate", "2026-07-31"))
				.andExpect(status().isUnauthorized());

		mockMvc.perform(post("/api/v1/training/readiness/daily/" + STATE_DATE)
						.with(auth(accountId))
						.with(csrf()))
				.andExpect(status().isBadRequest())
				.andExpect(jsonPath("$.code").value("DAILY_READINESS_STATE_SNAPSHOT_REQUIRED"));
		...
		MvcResult snapshotResult = mockMvc.perform(post("/api/v1/training/athlete-state/daily/" + STATE_DATE)
						.with(auth(accountId))
						.with(csrf())
						.contentType(MediaType.APPLICATION_JSON)
						.content("""
		...
		mockMvc.perform(post("/api/v1/training/readiness/assessments")
				...
				.andExpect(status().isOk())
				.andExpect(jsonPath("$.algorithmVersion").value("READINESS_V1"))
				.andExpect(jsonPath("$.contributions", hasSize(8)))
				.andExpect(jsonPath("$.summaryReasonCode").exists())
				.andExpect(jsonPath("$.recommendedAction").doesNotExist())
				.andExpect(jsonPath("$.medicalClearance").doesNotExist())
				.andExpect(jsonPath("$.injuryProbability").doesNotExist());
	...
	private AccountId athlete() {
		...
		createAthleteProfileUseCase.execute(
				...
				"Riley", "Chen", LocalDate.of(1993, 8, 8), Sex.FEMALE,
				Height.ofCentimeters(170), Weight.ofKilograms(62),
