...
public class DeleteWorkoutExerciseUseCase {
	...
	public DeleteWorkoutExerciseUseCase(
	...
	public void execute(
		...
		if (exercise.status() != WorkoutExerciseStatus.PLANNED) {
			throw new WorkoutExerciseDeleteNotAllowedException();
		}
		workoutExerciseRepository.delete(exercise);
		List<WorkoutExercise> remaining = workoutExerciseRepository.findAllByWorkoutDayIdAndAthleteId(
				day.id(), athleteId);
		WorkoutExerciseSupport.compactOrders(remaining, workoutExerciseRepository, day.id(), athleteId, clock);
	}
