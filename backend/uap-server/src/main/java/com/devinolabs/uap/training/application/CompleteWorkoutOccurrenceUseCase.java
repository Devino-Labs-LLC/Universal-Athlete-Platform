...
@Service
public class CompleteWorkoutOccurrenceUseCase {
	...
	public WorkoutOccurrenceDetailResult execute(
		...
		boolean hasIncomplete = executions.stream()
				.anyMatch(e -> e.status() == WorkoutExerciseExecutionStatus.NOT_STARTED
						|| e.status() == WorkoutExerciseExecutionStatus.IN_PROGRESS);
		if (hasIncomplete) {
			throw new WorkoutOccurrenceHasIncompleteExercisesException();
		}
		...
		return WorkoutOccurrenceSupport.toDetailResult(
				saved,
				WorkoutExerciseExecutionSupport.toResults(executions));
	}
