...
@SpringBootTest(properties = {
		"uap.billing.organization-capacity-enforcement.enabled=false",
		"uap.billing.entitlement-enforcement.enabled=true",
		"uap.billing.stripe.enabled=false"
})
@AutoConfigureMockMvc
@Import(TestcontainersConfiguration.class)
class OrganizationAthleteCapacityFlagOffHttpIntegrationTests {
	...
	@Test
	void capacityFalseAtBandMaximumStillAcceptsAndIgnoresClientEnableHeader() throws Exception {
		VerifiedAccount owner = accounts.registerVerified("cap-off-owner");
		VerifiedAccount athlete = accounts.registerVerifiedAthlete("cap-off-athlete");
		...
		mockMvc.perform(post("/api/v1/invitations/" + rawToken + "/accept")
						...
						.with(csrf())
						.header("UAP_BILLING_ORGANIZATION_CAPACITY_ENFORCEMENT_ENABLED", "true"))
