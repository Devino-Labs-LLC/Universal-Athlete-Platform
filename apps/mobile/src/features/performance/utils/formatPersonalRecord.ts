...
export function formatPerformanceMeasurement(
  measurement: PerformanceMeasurement | null | undefined,
): string | null {
  if (!measurement) {
    return null;
  }
...
export function formatPersonalRecord(record: PersonalRecord): string {
  switch (record.recordType) {
    ...
    case 'MOST_REPETITIONS_AT_WEIGHT': {
      ...
      const weight =
        record.weightValue != null
          ? formatWeight(record.weightValue, record.weightUnit)
    ...
    case 'LONGEST_DURATION': {
      const seconds =
        record.measuredValue ?? record.normalizedValue ?? record.repetitions ?? null;
