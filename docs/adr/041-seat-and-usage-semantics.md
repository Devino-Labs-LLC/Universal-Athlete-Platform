...
- Downgrade / initial purchase: **cannot become effective** while active billable count exceeds the target band; ORG_OWNER remediates; never automatically delete/remove athletes or memberships. Slice E / launch hardening must reject undersized bands (count 40 → not `ORG_BAND_25`). Request-time rejection is HTTP **409** `ORGANIZATION_PLAN_CAPACITY_CONFLICT` (not 402). A count that grows during PENDING Checkout is **not** closed by that preflight: the authoritative provider snapshot is still applied, members stay, and over-capacity follows §37 / §40. No period-end scheduled downgrade in the initial Slice E contract.  
