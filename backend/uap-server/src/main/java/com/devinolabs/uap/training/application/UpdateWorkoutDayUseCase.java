...
public class UpdateWorkoutDayUseCase {
	...
	public WorkoutDayResult execute(
		...
		try {
			...
			if (command.scheduledDayPresent()) {
				if (command.scheduledDay() == null) {
					throw new IllegalArgumentException("scheduledDay must not be null");
				}
				day.changeScheduledDay(command.scheduledDay(), clock);
			}
