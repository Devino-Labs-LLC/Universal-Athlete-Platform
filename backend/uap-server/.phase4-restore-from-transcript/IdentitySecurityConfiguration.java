...
import com.devinolabs.uap.identity.infrastructure.http.IdentityRateLimitFilter;
import com.devinolabs.uap.identity.infrastructure.http.InMemoryRateLimiter;
import com.devinolabs.uap.identity.infrastructure.http.RateLimiter;

/**
 * HTTP security boundary for Identity.
 *
 * <p><b>CSRF:</b> double-submit cookie ({@link CookieCsrfTokenRepository} +
 * {@link SpaCsrfTokenRequestHandler}). Register/verify/login are CSRF-ignored
 * (no session cookies yet). Refresh/logout/logout-all require {@code X-XSRF-TOKEN}.
 *
 * <p><b>CORS:</b> credentials only for explicitly configured origins; wildcards rejected
 * when credentials are enabled.
 *
 * <p><b>Sessions:</b> fully stateless; access/refresh carried by HTTP-only cookies
 * (or future bearer transport via {@link AuthTokenTransport}).
 */
@Configuration
...
class IdentitySecurityConfiguration {
	...
	static final String ME_PATH = "/api/v1/identity/me";
	...
	@Bean
	RateLimiter identityRateLimiter(IdentityHttpProperties httpProperties, java.time.Clock clock) {
		return new InMemoryRateLimiter(httpProperties.getRateLimit(), clock);
	}

	@Bean
	IdentityRateLimitFilter identityRateLimitFilter(RateLimiter identityRateLimiter) {
		return new IdentityRateLimitFilter(identityRateLimiter);
	}
	...
	SecurityFilterChain identitySecurityFilterChain(
			...
			AccessTokenAuthenticationFilter accessTokenAuthenticationFilter,
			IdentityRateLimitFilter identityRateLimitFilter) throws Exception {
		...
		http
				...
				.authorizeHttpRequests(auth -> auth
						...
						.requestMatchers(HttpMethod.GET, ME_PATH).authenticated()
						.requestMatchers(HttpMethod.POST, LOGOUT_PATH, LOGOUT_ALL_PATH).authenticated()
						.anyRequest().denyAll())
				.addFilterBefore(identityRateLimitFilter, UsernamePasswordAuthenticationFilter.class)
