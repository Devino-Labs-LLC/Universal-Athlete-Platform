...
@Service
public class StartWorkoutExerciseUseCase {
	...
	public StartWorkoutExerciseUseCase(
	...
	public WorkoutSessionResult execute(
		...
		WorkoutExercise exercise = WorkoutSessionSupport.requireOwnedExercise(
				workoutExerciseRepository, day.id(), athleteId, exerciseId);

		WorkoutSession session = WorkoutSessionSupport.getOrCreateSession(
				workoutSessionRepository, exercise, athleteId, clock);
		try {
			session.start(clock);
		}
