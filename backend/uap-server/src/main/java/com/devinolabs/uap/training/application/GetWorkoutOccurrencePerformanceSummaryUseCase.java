...
/**
 * Session-level performance roll-up for one occurrence.
 */
@Service
public class GetWorkoutOccurrencePerformanceSummaryUseCase {
	...
	public GetWorkoutOccurrencePerformanceSummaryUseCase(
	...
	public WorkoutOccurrencePerformanceResult execute(
			AccountId accountId,
		...
		List<WorkoutExerciseExecution> executions = workoutExerciseExecutionRepository
				.findAllByWorkoutOccurrenceIdAndAthleteId(occurrenceId, athleteId);
		...
		List<ExerciseExecutionPerformanceResult> exercises = executions.stream()
				.sorted(Comparator.comparingInt(WorkoutExerciseExecution::displayOrder)
						.thenComparing(execution -> execution.id().value()))
				.map(execution -> new ExerciseExecutionPerformanceResult(
						execution.id(),
						...
						ExercisePerformanceMetricCalculator.calculate(
								sets.getOrDefault(execution.id(), List.of()))))
				.toList();

		return new WorkoutOccurrencePerformanceResult(
				occurrence.id(),
				occurrence.scheduledDate(),
				occurrence.status(),
				occurrence.startedAt(),
				occurrence.completedAt(),
				TrainingPerformanceSupport.totals(exercises),
				exercises);
	}
